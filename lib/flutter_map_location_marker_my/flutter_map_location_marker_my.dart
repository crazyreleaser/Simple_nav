...
class LocationMarkerPlugin implements MapPlugin {
  ...
  ///The event stream for current position coodinates
  final StreamController<latlong.LatLng>? myPositionStreamController;
...
class _LocationMarkerLayerState extends State<LocationMarkerLayer>
  ...
  Widget _buildLocationMarker(Position position) {
    ...
    if(widget.plugin.myPositionStreamController != null) widget.plugin.myPositionStreamController!.add(latLng);
    return GroupLayer(
      GroupLayerOptions(
        group: [
                      ...
                      child: StreamBuilder(
                        ...
                        builder: (BuildContext context,
                          ...
                          if (snapshot.data?.heading != null) {
                            // print('inside map heading: '+ snapshot.data!.heading!.toString());
                            widget.plugin.headingStreamController!.add(snapshot.data!.heading!);
                            return Transform.rotate(
                              angle: geodesy.degToRadian(snapshot.data!.heading!),
                              child: CustomPaint(
  ...
  void _moveMap(latlong.LatLng latLng, double zoom) {
    _animationController?.dispose();
